...
def generate(prompt: str, max_tokens: int = 800, temperature: float = 0.3) -> str:
    if MOCK_MODE:
        return '''{
    ...
    try:
        ...
        try:
            ...
            try:
                return generate_with_openai(prompt, max_tokens, temperature)
            except Exception as e2:
                raise Exception(f"All LLMs failed. Groq: {e_groq}. Gemini: {e}. OpenAI: {e2}")


def generate_with_groq(prompt: str, max_tokens: int = 800, temperature: float = 0.3) -> str:
...
def generate_with_gemini(prompt: str, max_tokens: int = 800, temperature: float = 0.3) -> str:
    max_retries = 3
    for attempt in range(max_retries):
        try:
            ...
            if ("503" in error_str or "429" in error_str) and attempt < max_retries - 1:
                wait = (attempt + 1) * 10
                print(f"[INFO] Gemini busy, retrying in {wait}s (attempt {attempt + 1}/{max_retries})...")
                time.sleep(wait)
                continue
            raise


def generate_with_openai(prompt: str, max_tokens: int = 800, temperature: float = 0.3) -> str:
