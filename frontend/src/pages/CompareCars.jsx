...
const CompareCars = () => {
  ...
  const [freeCar1, setFreeCar1] = useState('')
  const [freeCar2, setFreeCar2] = useState('')
  const [freeQuery, setFreeQuery] = useState('')
  const [freeAnswer, setFreeAnswer] = useState('')
  const [isLoadingFree, setIsLoadingFree] = useState(false)

  const [paidCar1, setPaidCar1] = useState('')
  const [paidCar2, setPaidCar2] = useState('')
  const [isLoadingPaid, setIsLoadingPaid] = useState(false)
  const [paidError, setPaidError] = useState('')

  const getUniqueModelsList = () => {
    const list = []
    carsData.forEach(c => {
      const label = `${c.brand} ${c.model}`
      if (!list.some(item => item.label === label)) {
        list.push({ label, brand: c.brand, model: c.model, variant: c.variant, fullCarObj: c })
      }
    })
    return list
  }
  const uniqueModels = getUniqueModelsList()
  
  useEffect(() => {
    if (uniqueModels.length > 1) {
      if (!freeCar1) setFreeCar1(uniqueModels[0].label)
      if (!freeCar2) setFreeCar2(uniqueModels[1].label)
      if (!paidCar1) setPaidCar1(uniqueModels[0].label)
      if (!paidCar2) setPaidCar2(uniqueModels[1].label)
    }
  }, [carsData])

  const cleanCarForAi = (carObj) => {
    if (!carObj) return null
    const specs = carObj.specifications || {}
    return {
      brand: carObj.brand,
      model: carObj.model,
      variant: carObj.variant,
      price: carObj.price || "",
      engine: carObj.engine || "",
      transmission: specs.transmission || specs.Transmission || "",
      mileage: carObj.mileage || "",
      fuelType: specs.fuelType || specs.FuelType || "",
      safetyRating: specs.safetyRating || specs.SafetyRating || "",
  ...
  const cleanTextSymbols = (str) => {
    if (!str) return ""
    return str.replace(/₹/g, "Rs.").replace(/¹/g, "Rs.").replace(/\u20B9/g, "Rs.")
  }

  const generateBuyingGuidePdf = (car1, car2, reportData) => {
    const doc = new jsPDF()
    
    const primaryColor = [252, 100, 28] // Orange theme
    ...
    doc.setFontSize(28)
    doc.text("DRYVSQUAD AI ADVISOR", 20, 130)
    
    doc.setFont("helvetica", "normal")
    doc.setFontSize(18)
    doc.text("Premium Detailed Buying Guide", 20, 142)
    
    doc.setTextColor(...primaryColor)
    doc.setFont("helvetica", "bold")
    doc.setFontSize(22)
    const comparedCarsText = `${getDisplayName(car1)} vs ${getDisplayName(car2)}`
    const splitComparedText = doc.splitTextToSize(comparedCarsText, 170)
    doc.text(splitComparedText, 20, 170)
    
    doc.setTextColor(200, 200, 200)
    doc.setFont("helvetica", "italic")
    doc.setFontSize(11)
    doc.text("Generated by DryvSquad AI - Unlocked Premium Report", 20, 260)
    doc.text(`Date: ${new Date().toLocaleDateString()}`, 20, 268)
    
    const drawDecoration = (pageNumber) => {
      // Header line
      ...
      doc.setFont("helvetica", "bold")
      doc.setFontSize(8)
      doc.text("DRYVSQUAD AI ADVISOR - DETAILED BUYING GUIDE", 15, 11)
      
      // Footer line
      doc.line(15, 282, 195, 282)
      doc.setFont("helvetica", "normal")
      doc.text("Generated by DryvSquad AI Advisor. For evaluation purposes only.", 15, 287)
      doc.text(`Page ${pageNumber} of 3`, 180, 287)
    }

    // Page 2: Personalized Comparison & Financials
    doc.addPage()
    doc.setFillColor(248, 250, 252) // Off white bg
    doc.rect(0, 0, 210, 297, 'F')
    drawDecoration(2)
    
    const drawHeader = (title, y) => {
      doc.setFillColor(...primaryColor)
      doc.rect(15, y - 6, 4, 8, 'F')
    ...
    let currentY = 32
    drawHeader("1. HEAD-TO-HEAD PERSONALIZED COMPARISON", currentY)
    currentY += 10
    ...
    const textCompare = doc.splitTextToSize(cleanTextSymbols(reportData.personalizedComparison) || "", 175)
    doc.text(textCompare, 20, currentY)
    currentY += textCompare.length * 5 + 15
    
    drawHeader("2. TCO & EMIS FINANCIAL ANALYSIS", currentY)
    currentY += 10
    
    doc.setFont("helvetica", "normal")
    doc.setFontSize(10)
    const textFinance = doc.splitTextToSize(cleanTextSymbols(reportData.financialAnalysis) || "", 175)
    doc.text(textFinance, 20, currentY)
    currentY += textFinance.length * 5 + 12
    
    // Financial Highlights Cards
    doc.setFillColor(255, 255, 255)
    doc.setDrawColor(226, 232, 240)
    doc.roundedRect(20, currentY, 80, 36, 3, 3, 'FD')
    doc.roundedRect(110, currentY, 80, 36, 3, 3, 'FD')
    
    doc.setTextColor(...primaryColor)
    doc.setFont("helvetica", "bold")
    doc.setFontSize(10)
    const splitName1 = doc.splitTextToSize(getDisplayName(car1), 70)
    doc.text(splitName1, 25, currentY + 7)
    
    const splitName2 = doc.splitTextToSize(getDisplayName(car2), 70)
    doc.text(splitName2, 115, currentY + 7)
    
    doc.setTextColor(...darkColor)
    doc.setFont("helvetica", "normal")
    doc.setFontSize(9)
    doc.text(`Est. 5-Yr TCO: ${cleanTextSymbols(reportData.car1_tco) || "N/A"}`, 25, currentY + 20)
    doc.text(`Est. Monthly EMI: ${cleanTextSymbols(reportData.car1_emi) || "N/A"}`, 25, currentY + 28)
    
    doc.text(`Est. 5-Yr TCO: ${cleanTextSymbols(reportData.car2_tco) || "N/A"}`, 115, currentY + 20)
    doc.text(`Est. Monthly EMI: ${cleanTextSymbols(reportData.car2_emi) || "N/A"}`, 115, currentY + 28)
    
    currentY += 45
    
    // Page 3: Insurance & Resale
    doc.addPage()
    ...
    currentY = 32
    drawHeader("3. INSURANCE RECOMMENDATIONS & ADD-ONS", currentY)
    currentY += 10
    
    doc.setTextColor(51, 65, 85)
    doc.setFont("helvetica", "normal")
    doc.setFontSize(10)
    const textInsurance = doc.splitTextToSize(cleanTextSymbols(reportData.insuranceRecommendations) || "", 175)
    doc.text(textInsurance, 20, currentY)
    currentY += textInsurance.length * 5 + 15
    
    drawHeader("4. RESALE VALUE RETENTION ANALYSIS", currentY)
    currentY += 10
    
    doc.setFont("helvetica", "normal")
    doc.setFontSize(10)
    const textResale = doc.splitTextToSize(cleanTextSymbols(reportData.resaleValuePredictions) || "", 175)
    doc.text(textResale, 20, currentY)
    currentY += textResale.length * 5 + 15
    
    // Resale Table
    drawHeader("EXPECTED RESALE VALUE RETENTION % (YEARS 1-5)", currentY)
    currentY += 12
    ...
    doc.setFont("helvetica", "bold")
    doc.setFontSize(9)
    doc.text("Car Model", 24, currentY + 6)
    doc.text("Yr 1", 90, currentY + 6)
    doc.text("Yr 2", 110, currentY + 6)
    doc.text("Yr 3", 130, currentY + 6)
    doc.text("Yr 4", 150, currentY + 6)
    doc.text("Yr 5", 170, currentY + 6)
    ...
    doc.rect(20, currentY, 170, 8, 'F')
    doc.setFont("helvetica", "normal")
    doc.text(getDisplayName(car1), 24, currentY + 6)
    const r1 = reportData.car1_resale || [90, 80, 70, 60, 50]
    r1.forEach((pct, idx) => {
      doc.text(`${pct}%`, 90 + idx * 20, currentY + 6)
    })
    ...
    doc.rect(20, currentY, 170, 8, 'F')
    doc.text(getDisplayName(car2), 24, currentY + 6)
    const r2 = reportData.car2_resale || [92, 82, 72, 62, 52]
    r2.forEach((pct, idx) => {
      doc.text(`${pct}%`, 90 + idx * 20, currentY + 6)
    })
    
    doc.save(`${getDisplayName(car1).replace(/\s+/g, '_')}_vs_${getDisplayName(car2).replace(/\s+/g, '_')}_Buying_Guide.pdf`)
  }

  const handleFreeSubmit = async (e) => {
    e.preventDefault()
    if (!freeCar1 || !freeCar2 || !freeQuery.trim()) return
    setIsLoadingFree(true)
    setFreeAnswer('')
    try {
      const c1Obj = carsData.find(c => `${c.brand} ${c.model}` === freeCar1)
      const c2Obj = carsData.find(c => `${c.brand} ${c.model}` === freeCar2)
      
      const cleanCar1 = cleanCarForAi(c1Obj)
      const cleanCar2 = cleanCarForAi(c2Obj)

      const response = await api.getAiComparison(cleanCar1, cleanCar2, 'free', freeQuery)
      if (response && response.success && response.data) {
        setFreeAnswer(response.data.comparison_text || 'No response returned.')
      } else {
        setFreeAnswer('Failed to retrieve AI analysis. Please try again.')
      }
    } catch (err) {
      console.error(err)
      setFreeAnswer('Network error occurred. Please try again.')
    } finally {
  ...
  const handlePaidSubmit = async (e) => {
    e.preventDefault()
    if (!paidCar1 || !paidCar2) return
    setIsLoadingPaid(true)
    setPaidError('')
    try {
      const c1Obj = carsData.find(c => `${c.brand} ${c.model}` === paidCar1)
      const c2Obj = carsData.find(c => `${c.brand} ${c.model}` === paidCar2)
      
      const cleanCar1 = cleanCarForAi(c1Obj)
      const cleanCar2 = cleanCarForAi(c2Obj)

      const response = await api.getAiComparison(cleanCar1, cleanCar2, 'paid')
      if (response && response.success && response.data) {
        generateBuyingGuidePdf(cleanCar1, cleanCar2, response.data)
      } else {
        setPaidError('Failed to generate report. Please try again.')
      }
    } catch (err) {
      console.error(err)
      setPaidError('Network error occurred. Please try again.')
    } finally {
  ...
  // Auto-select car from query params
  useEffect(() => {
    if (carsData.length === 0) return
    const params = new URLSearchParams(window.location.search)
    const select1 = params.get('car1')
    const select2 = params.get('car2')
    if (select1) {
      const match = carsData.find(
        (c) =>
          c.model.toLowerCase() === select1.toLowerCase() ||
          `${c.brand} ${c.model}`.toLowerCase() === select1.toLowerCase()
      )
      if (match) {
        setCar1Id(match.id)
        if (select2) {
          const match2 = carsData.find(
            (c) =>
              c.model.toLowerCase() === select2.toLowerCase() ||
              `${c.brand} ${c.model}`.toLowerCase() === select2.toLowerCase()
          )
          if (match2) {
            setCar2Id(match2.id)
            executeComparison(match.id, match2.id)
          }
        }
      }
    }
