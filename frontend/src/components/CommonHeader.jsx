// src/components/CommonHeader.jsx - Optimized for instant theme toggle
/*
================================================================================
File Name : CommonHeader.jsx
Author : Tahseen Raza
Created Date : 2025-01-15
Description : Optimized header component with instant theme support
...
// Static category data - moved outside component for performance
...
const CommonHeader = () => {
  ...
  const textColor = useMemo(() => isDark ? 'text-white' : 'text-gray-900', [isDark])
  const textHoverColor = useMemo(() => isDark ? 'hover:text-yellow-400' : 'hover:text-gray-700', [isDark])
  ...
  // Close menu on location change - optimized
  useEffect(() => {
    setIsOpen(false)
  ...
  // Desktop Categories Dropdown Component - memoized
  const CategoriesDropdown = useCallback(() => {
    ...
    return (
      ...
      >
        <button className={`flex items-center gap-1 font-semibold text-sm xl:text-[16px] tracking-wide ${catTextColor} ${catHoverText} transition-none`}>
          Categories
        ...
        {isCatOpen && (
          ...
          >
            <Link
              to="/compare-cars"
              className={`flex items-center justify-between px-3 sm:px-4 py-2.5 sm:py-3 ${catHoverBg} transition-none border-b ${catBorderColor}`}
            ...
            {DESKTOP_CATEGORIES.map((category, idx) => (
              ...
              >
                <div className={`flex items-center justify-between px-3 sm:px-4 py-2.5 sm:py-3 ${catHoverBg} cursor-pointer transition-none`}>
                  <div>
                    ...
                    <div className="py-2">
                      ...
                      {category.articles.map((article, articleIdx) => (
                        <Link
                          ...
                          className={`block px-3 sm:px-4 py-1.5 sm:py-2 ${catHoverBg} transition-none`}
                          onClick={() => setIsCatOpen(false)}
                        >
        ...
        <div className="flex items-center justify-between">
            ...
            {NAV_LINKS.map((link) => (
              <NavLink
                ...
                className={({ isActive }) => {
                  ...
                  return `font-semibold text-sm xl:text-[16px] tracking-wide transition-none ${
          ...
          >
            <div className="w-5 sm:w-6 h-4 sm:h-5 flex flex-col justify-between">
              <span
                className={`h-0.5 w-full bg-current transition-all duration-150 ${
              ...
              <div className="pt-3 pb-4 space-y-1.5">
                {NAV_LINKS.map((link) => (
                  <NavLink
                    ...
                    className={({ isActive }) => {
                      ...
                      return `block py-2 font-medium text-base sm:text-lg transition-none ${
                        isActive ? activeClass : `${textColor} ${textHoverColor}`
                      }`
                    }}
                    onClick={closeMenu}
                ...
                <div className="py-1">
                  <button
                    ...
                    className={`w-full flex items-center justify-between py-2 font-medium text-base sm:text-lg ${textColor} ${textHoverColor} transition-none`}
                  >
                            ...
                            if (item.isDirect) {
                              return (
                                <Link
                                  key={idx}
                                  to={item.path}
                                  className={`block py-2 text-sm sm:text-base ${textColor} ${textHoverColor} transition-none pl-2 border-l-2 border-transparent hover:border-gray-400`}
                                  onClick={closeMenu}
                                >
                                  {item.name}
                                </Link>
                            ...
                            return (
                              <div key={idx} className="space-y-1">
                                <button
                                  ...
                                  className={`w-full flex items-center justify-between py-2 text-sm sm:text-base ${textColor} ${textHoverColor} transition-none pl-2 border-l-2 border-transparent hover:border-gray-400`}
                                >
                                  <span>{item.name}</span>
                                  <svg
                                      ...
                                      <div className="ml-4 sm:ml-6 space-y-1 pb-1">
                                        {item.articles.map((article, articleIdx) => (
                                          <Link
                                            key={articleIdx}
                                            to={`/article/${article.slug}`}
                                            className={`block py-1.5 text-xs sm:text-sm ${textColor} ${textHoverColor} transition-none pl-2 border-l-2 border-transparent hover:border-gray-400`}
                                            onClick={closeMenu}
                                          >
                                            {article.title}
                                          </Link>
