...
// ========================================
// Data-driven so it renders 2 OR 3 cars (CarDekho-style optional 3rd car)
// without duplicating the whole layout per car slot.
// ========================================
const ComparisonResults = ({
  comparisonData,
  onClear,
  ...
  const { car1, car2, car3, summary } = comparisonData

  // Build the ordered list of cars actually being compared (2 or 3).
  const cars = [
    { car: car1, position: 1, ref: carCardRef1 },
    { car: car2, position: 2, ref: carCardRef2 },
    car3 ? { car: car3, position: 3, ref: carCardRef3 } : null,
  ].filter(Boolean)

  const columnCount = cars.length // 2 or 3
  // Desktop layout reserves a leading "Parameter" label column; exposed as
  // a CSS custom property so the md: breakpoint can pick it up in pure
  // CSS instead of checking window.innerWidth in JS (which wouldn't react
  // to resizing).
  const gridVars = { '--cmp-cols': `repeat(${columnCount}, minmax(0, 1fr))` }
  const desktopGridClass = "md:[grid-template-columns:minmax(0,0.9fr)_var(--cmp-cols)]"
  ...
  const getColorClasses = (rating) => {
    ...
    if (rating >= 8) {
      return { bg: 'bg-green-500', text: 'text-green-600 dark:text-green-400', bar: 'bg-green-500' }
    } else if (rating >= 5) {
      return { bg: 'bg-yellow-500', text: 'text-yellow-600 dark:text-yellow-400', bar: 'bg-yellow-500' }
    } else {
      return { bg: 'bg-red-500', text: 'text-red-600 dark:text-red-400', bar: 'bg-red-500' }
    }
  }

  // Returns { label, icon, byPosition: { 1: {...}, 2: {...}, 3?: {...} } }
  const getRatingData = (key) => {
    const byPosition = {}
    for (const { car, position } of cars) {
      const r = car.ratings?.[key] || null
      byPosition[position] = {
    ...
    const anyRating = cars.map(({ position }) => byPosition[position]).find(r => r?.rating != null)
    return {
      label: categoryOrder.find(c => c.key === key)?.label || key,
      icon: categoryOrder.find(c => c.key === key)?.icon || '📊',
      byPosition,
    }
  }
  ...
  const renderRatingBar = (rating) => {
    ...
    return (
      <div className="flex items-center justify-center gap-1.5 sm:gap-3 w-full">
        <div className="w-14 sm:w-24 md:w-36 h-2 sm:h-2.5 bg-gray-200 dark:bg-dark-700 rounded-full overflow-hidden theme-transition">
          <motion.div
            initial={{ width: 0 }}
            animate={{ width: `${percentage}%` }}
            transition={{ duration: 0.8, ease: 'easeOut' }}
        ...
        <span className={`text-[11px] sm:text-sm font-bold ${colors.text} min-w-[34px] sm:min-w-[46px] text-right theme-transition`}>
          {rating !== null ? `${rating.toFixed(1)}/10` : 'N/A'}
        </span>
      </div>
    )
  ...
  const getVisibleCategories = () => {
    ...
    for (const cat of categoryOrder) {
      const data = getRatingData(cat.key)
      const hasData = cars.some(({ position }) => data.byPosition[position]?.rating !== null)
      if (hasData) {
        visible.push({ ...cat, data })
      }
    }
  ...
  // Mobile: 2 cars stack side by side (existing look), 3 cars wrap to a
  // single column per card so nothing gets crushed on small screens.
  const mobileGridCols = columnCount === 2 ? 'grid-cols-2' : 'grid-cols-1 sm:grid-cols-3'
  ...
  return (
    <div className="bg-white dark:bg-dark-800 rounded-xl shadow-lg overflow-hidden theme-transition">
      ...
      <div
        id="comparison-car-cards"
        className={`px-3 sm:px-6 md:px-8 py-4 sm:py-6 md:py-8 grid ${mobileGridCols} ${desktopGridClass} gap-2 sm:gap-4 md:gap-6 bg-gradient-to-b from-gray-50 to-white dark:from-dark-800/80 dark:to-dark-800 border-b border-gray-200 dark:border-dark-700 theme-transition`}
        style={gridVars}
      >
        {/* Parameter Label — desktop only */}
        <div className="hidden md:flex flex-col items-center justify-center text-center">
          <span className="text-gray-400 dark:text-gray-500 font-bold tracking-widest uppercase text-sm theme-transition">Parameter</span>
        </div>

        {cars.map(({ car, position, ref }, idx) => {
          const carFull = `${car.brand} ${car.model}`
          const carVariant = car.name || car.variant || ''
          return (
            <div
              key={position}
              ref={ref}
              className={`text-center flex flex-col items-center justify-end md:justify-start relative px-1 sm:px-0 ${
      ...
      <div className="p-4 md:p-6 bg-gray-50/50 dark:bg-dark-900/20 theme-transition">
        <div className="space-y-2">
          {visibleCategories.map((category) => {
            const { key, label, icon, data } = category
            const isExpanded = expandedRow === key
            const hasExplanation = cars.some(({ position }) => data.byPosition[position]?.explanation)

            // Highest rating among comparable cars in this row, for the
            // subtle "best in row" star badge in the expanded detail view.
            const ratingsThisRow = cars
              .map(({ position }) => data.byPosition[position]?.rating)
              .filter((r) => r !== null && r !== undefined)
            const maxRatingThisRow = ratingsThisRow.length ? Math.max(...ratingsThisRow) : null
            ...
            return (
              ...
              >
                <div
                  className={`px-3 sm:px-6 md:px-8 py-4 md:py-6 grid ${mobileGridCols} ${desktopGridClass} gap-2 sm:gap-4 md:gap-6 items-center`}
                  style={gridVars}
                >
                  <div className={`flex items-center gap-2 sm:gap-4 ${columnCount === 2 ? 'col-span-2' : 'col-span-full sm:col-span-1'} md:col-span-1`}>
                    <div className="flex-shrink-0 flex items-center justify-center w-9 h-9 sm:w-12 sm:h-12 rounded-xl bg-gray-50 dark:bg-dark-700 border border-gray-100 dark:border-dark-600 shadow-sm text-lg sm:text-2xl theme-transition">
                      {icon}
                    </div>
                    <span className="font-bold text-xs sm:text-sm text-gray-800 dark:text-gray-200 uppercase tracking-wide theme-transition">
                  ...
                  {cars.map(({ position }) => (
                    <div key={position} className="flex flex-col items-center justify-center">
                      <span className="text-sm sm:text-base font-semibold text-gray-800 dark:text-gray-200 mb-1.5 sm:mb-2 text-center theme-transition">
                        {data.byPosition[position]?.value ?? 'N/A'}
                      </span>
                      {renderRatingBar(data.byPosition[position]?.rating ?? null)}
                    </div>
                  ))}
                </div>
                      ...
                      <div className="px-3 sm:px-6 md:px-8 py-4 md:py-6 bg-yellow-50/30 dark:bg-yellow-900/5 border-t border-yellow-100 dark:border-yellow-900/20 theme-transition">
                        ...
                        >
                          ...
                          {cars.map(({ car, position }) => {
                            ...
                            const rating = data.byPosition[position]?.rating
                            if (!explanation) return <div key={position} className="hidden sm:block" />
                            const carFull = `${car.brand} ${car.model}`
                            const carVariant = car.name || car.variant || ''
                            return (
                              <div key={position} className="p-2.5 sm:p-4 rounded-xl border border-gray-200 dark:border-dark-700 bg-white dark:bg-dark-800 shadow-sm theme-transition">
                                <div className="flex items-center gap-1.5 sm:gap-2 mb-1.5 sm:mb-2">
                                  <span className="text-xs sm:text-sm font-bold text-gray-800 dark:text-gray-200 theme-transition truncate">
                                    {carFull} ({carVariant})
                                  </span>
                                  {rating !== null && maxRatingThisRow !== null && rating === maxRatingThisRow && ratingsThisRow.length > 1 && (
                                    <span className="text-[9px] sm:text-[10px] font-bold uppercase tracking-wider bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-400 px-1.5 sm:px-2 py-0.5 rounded-full flex-shrink-0 theme-transition">
                                      ★ Best
                                    </span>
                                  )}
