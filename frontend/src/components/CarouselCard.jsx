...
const CarouselCard = ({
  ...
  const handleImageError = (e) => {
    ...
    // If no fallback exists, stop here
    if (!fallbackImage) return

    // Don't set the same fallback again
    const currentSrc = img.getAttribute('src')

    if (currentSrc === fallbackImage) return
  ...
  return (
    <article
      className={`group flex-shrink-0 w-[calc((100vw-3rem)/3)] sm:w-[240px] md:w-[260px] lg:w-[280px] rounded-xl overflow-hidden will-change-transform ${cardShadowClass} transition-shadow duration-300 ease-out ${
